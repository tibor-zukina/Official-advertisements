...
if (isset($_POST['id']) && isset($_SESSION["authTokenAdmin"]) ) {
    ...
    $stmt = $mysqli->stmt_init();
    $stmt->prepare("UPDATE Usr SET DeletedUser = 'yes', ApproveTokenUser = ? WHERE HashUser LIKE ? ");
    ...
    $response["success"] = 1;
    $response["message"] = "User successfully rejected";

    $nameEmail = $name.' '.$surname;

    rejectUser($userId, $email, $nameEmail);
