...
function fillTable() {
   ...
   var newHtml = '<tr> <th class="narrow"> # </th> <th> Name </th> <th> Url </th> <th> Ad </th> <th class="noBorder" > Time </th> </tr>';
   for(i = currentPage * 20; i < Math.min(totalAds, (currentPage+1) * 20) ; i++) {
      var htmlStr = '<tr>' + '<td class="narrow">' + userAds[i].index + '</td>' + '<td>' + userAds[i].name + '</td>' + '<td>' + userAds[i].url + '</td>' + '<td>' + userAds[i].identificator + '</td>' + '<td class="noBorder">' + userAds[i].time + '</td>' + '</tr>';
      newHtml += htmlStr;
   }
...
function getLimitedAds(){
   ...
   $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    ...
                    totalAds = ads.length;
                    adsTable = document.getElementById('adsTable');
                    adsTable.innerHTML = '<tr> <th class="narrow"> # </th> <th> Name </th> <th> Url </th> <th> Ad </th> <th> Time </th> </tr>';
                    userAds = new Array();
                    ...
                    for (var i = 0; i < totalAds; i++) {
                       ...
                       ad = new Ad(index,name,url,identificator,time,id);
                       userAds.push(ad);
                       var htmlStr = '<tr>' + '<td class="narrow">' + index + '</td>' + '<td>' + name + '</td>' + '<td>' + url + '</td>' + '<td>' + identificator + '</td>' + '<td>' + time + '</td>' + '</tr>';
                       adsTable.innerHTML += htmlStr;
                    }
